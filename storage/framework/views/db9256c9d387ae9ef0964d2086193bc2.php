...
<head>
    ...
    <title>Konsultasi VIP — <?php echo e($inquiry->car_model ?? 'Kendaraan VIP'); ?> — Apex Automotive</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        <?php $viewerRole = auth()->user()->role; ?>
        <?php if($viewerRole === 'rm' || $viewerRole === 'manager'): ?>
        ...
        * { scrollbar-color: #0284c7 #080810 !important; }
        <?php elseif($viewerRole === 'delivery'): ?>
...
<body>
    <nav class="portal-nav">
        <?php if(auth()->user()->isDelivery()): ?>
        <a href="<?php echo e(route('delivery.portal')); ?>" class="nav-back">
            <i class="fa-solid fa-arrow-left"></i>
            <span>Portal Driver</span>
        </a>
        <?php else: ?>
        <a href="<?php echo e(route('portal.dashboard')); ?>" class="nav-back">
            <i class="fa-solid fa-arrow-left"></i>
            <span>Portal Saya</span>
        </a>
        <?php endif; ?>
        <div class="nav-title">
            Konsultasi VIP &nbsp;·&nbsp; <strong><?php echo e((auth()->user()->isRm() || auth()->user()->isManager() || auth()->user()->isDelivery()) ? $inquiry->name . ' (' . $inquiry->phone . ')' : ($inquiry->car_model ?? 'Kendaraan VIP')); ?></strong>
        </div>
        <div class="flex items-center gap-3">
            ...
            <span class="status-badge <?php echo e($inquiry->statusColor()); ?>" id="statusBadge"><?php echo e($inquiry->statusLabel()); ?></span>
        </div>
    </nav>
        ...
        <div style="width: 100%; max-width: 640px; background: #0c0c12; border: 1px solid rgba(239, 68, 68, 0.4); padding: 28px; border-radius: 4px; position: relative; box-shadow: 0 25px 60px rgba(0,0,0,0.9); font-family: 'Inter', sans-serif;">
            ...
            <div style="display: flex; align-items: center; gap: 14px; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                ...
                <div>
                    ...
                    <p style="font-size: 11px; font-family: monospace; color: #9ca3af; margin-top: 4px;">No: SPA/APEX/2026/0<?php echo e($inquiry->id); ?> &nbsp;·&nbsp; Unit: <?php echo e($inquiry->car_model); ?></p>
                </div>
            </div>
            ...
            <div style="max-height: 220px; overflow-y: auto; padding: 14px; background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.06); border-radius: 2px; margin-bottom: 20px; line-height: 1.6; font-size: 13px; color: #d1d5db;">
                ...
                <div style="margin-bottom: 14px;">
                    ...
                    <p style="margin: 0; color: #9ca3af;">Pembeli berhak menerima unit kendaraan <strong style="color: #ffffff;"><?php echo e($inquiry->car_model); ?></strong> sesuai spesifikasi kustomisasi yang telah disepakati. Pembeli berkewajiban melakukan pelunasan pembayaran sesuai skema penawaran resmi.</p>
                </div>
            ...
            <!-- Signature Form -->
            <form method="POST" action="<?php echo e(route('portal.contract.sign', $inquiry)); ?>" id="esignForm" onsubmit="saveCanvasSignature()">
                <?php echo csrf_field(); ?>
        ...
        <div style="width: 100%; max-width: 580px; background: #0c0c12; border: 1px solid rgba(59, 130, 246, 0.4); padding: 28px; border-radius: 4px; position: relative; box-shadow: 0 25px 60px rgba(0,0,0,0.9); font-family: 'Inter', sans-serif;">
            ...
            <div style="display: flex; align-items: center; gap: 14px; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                ...
                <div>
                    ...
                    <p style="font-size: 11px; font-family: monospace; color: #9ca3af; margin-top: 4px;">Instruksi Rekening Terproteksi &nbsp;·&nbsp; Unit: <?php echo e($inquiry->car_model); ?></p>
                </div>
            </div>
    ...
    <div class="consultation-layout">
        ...
        <aside class="sidebar">
            ...
            <div style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.08); padding: 14px; border-radius: 4px;">
                ...
                <div style="width: 100%; height: 140px; border-radius: 4px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 12px; background: #000;">
                    <img src="<?php echo e($carImg); ?>" alt="<?php echo e($inquiry->car_model); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                </div>

                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
                    <p class="car-title"><?php echo e($inquiry->car_model ?? '—'); ?></p>
                    <span style="font-family: 'Space Mono', monospace; font-size: 10px; font-weight: 700; color: #dc2626; background: rgba(220,38,38,0.15); border: 1px solid rgba(220,38,38,0.4); padding: 2px 8px; border-radius: 2px;">
                        ID: #APX-<?php echo e(str_pad($inquiry->id, 5, '0', STR_PAD_LEFT)); ?>
                ...
                <p style="font-family: 'Space Mono', monospace; font-size: 11px; color: #9ca3af; margin-top: 4px; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 6px;">
                    SKU: <span style="color: #e5e7eb; font-weight: 700;">SUPERCAR-<?php echo e(strtoupper(substr(md5($inquiry->car_model ?? 'VIP'), 0, 6))); ?></span>
                </p>
                <?php if($inquiry->selected_config): ?>
                    <p style="font-size: 11px; color: #9ca3af; margin-top: 4px;">
                        Spec: <span style="color: #e5e7eb;"><?php echo e($inquiry->selected_config); ?></span>
                    </p>
                <?php endif; ?>
            </div>
                ...
                <div style="background: rgba(249,115,22,0.08); border: 1px solid rgba(249,115,22,0.35); padding: 14px; border-radius: 4px;">
                    ...
                    <div style="font-size: 11px; color: #e5e7eb; line-height: 1.8;">
                        ...
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 6px; margin-bottom: 6px;">
                            ...
                            <span style="color: #fff; font-weight: 700; text-align: right; max-width: 160px;"><?php echo e($inquiry->car_model); ?></span>
                        </div>
                        
                        <?php if($inquiry->selected_color): ?>
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 6px; margin-bottom: 6px;">
                            ...
                            <span style="display: inline-flex; align-items: center; gap: 5px; font-weight: 700; color: #fff;">
                                <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.3); background: <?php echo e($inquiry->selected_color); ?>;"></span>
                                <?php echo e($inquiry->selected_color); ?>
                        ...
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 6px; margin-bottom: 6px;">
                            ...
                            <span style="display: flex; flex-direction: column; align-items: flex-end; gap: 3px; max-width: 160px; text-align: right;">
                                <?php $__currentLoopData = explode(',', $inquiry->selected_config); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $configItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 6px; margin-bottom: 6px;">
                            ...
                            <span style="color: #fff; font-weight: 700;"><?php echo e($inquiry->name); ?></span>
                        </div>
                        
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 6px; margin-bottom: 6px;">
                            <span style="color: #9ca3af;">Tujuan:</span>
                            <span style="color: #e5e7eb; text-align: right; max-width: 160px; font-size: 10px;"><?php echo e(auth()->user()->address ?? 'Alamat KYC Terverifikasi'); ?></span>
                        </div>
            ...
            <div id="salesSidebarContent" style="display: flex; flex-direction: column; gap: 16px;">
                <div>
                    <div class="detail-row">
                        ...
                        <span class="detail-value"><?php echo e($inquiry->name); ?></span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">No. HP / WhatsApp</span>
                        <span class="detail-value"><?php echo e($inquiry->phone); ?></span>
                    </div>
                    <?php if($inquiry->assigned_rm_name): ?>
                        <div class="detail-row">
                            <span class="detail-label">Sales RM</span>
                            <span class="detail-value"><?php echo e($inquiry->assigned_rm_name); ?></span>
                        </div>
                    <?php endif; ?>
                    <div class="detail-row" style="border-bottom:none;">
                        <span class="detail-label">Dibuat</span>
                        <span class="detail-value"><?php echo e($inquiry->created_at->format('d M Y, H:i')); ?></span>
                    </div>
                </div>
                ...
                <div style="background: rgba(20, 20, 28, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); padding: 16px; border-radius: 4px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                        ...
                        <a href="<?php echo e(route('profile.complete')); ?>" style="font-size: 11px; color: #fbbf24; text-decoration: underline; font-weight: 600;">Edit Profil</a>
                    </div>
                    ...
                    <?php if(auth()->user()->hasCompletedProfile()): ?>
                        ...
                        <div style="font-size: 12px; color: #9ca3af; border-top: 1px solid rgba(255, 255, 255, 0.08); padding-top: 10px; line-height: 1.6;">
                            <div style="margin-bottom: 4px;">
                                <strong style="color: #d1d5db;">NIK:</strong> <span style="color: #e5e7eb;"><?php echo e(auth()->user()->nik ?? '—'); ?></span>
                            </div>
                            <div>
                                <strong style="color: #d1d5db;">Alamat:</strong>
                                <p style="color: #9ca3af; margin-top: 2px; word-break: break-word;"><?php echo e(auth()->user()->address ?? '—'); ?></p>
                            </div>
                        </div>
                    <?php else: ?>
                        ...
                        <a href="<?php echo e(route('profile.complete')); ?>" style="display: block; text-align: center; width: 100%; padding: 10px; background: rgba(251, 191, 36, 0.15); border: 1px solid rgba(251, 191, 36, 0.4); color: #fde047; font-size: 11px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; text-decoration: none; border-radius: 2px;">
                            ISI DATA LEGALITAS
                        </a>
                    <?php endif; ?>
                </div>
                    ...
                    <?php if($inquiry->buyer_signed): ?>
                        <div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); padding: 12px; border-radius: 4px;">
                            ...
                            <p style="font-size: 11px; color: #9ca3af; margin-bottom: 10px;">
                                Ditandatangani pada: <span style="color: #e5e7eb;"><?php echo e($inquiry->buyer_signed_at?->format('d M Y, H:i')); ?></span>
                            </p>
                            ...
                            <a href="<?php echo e(route('portal.contract.download', $inquiry)); ?>" target="_blank" style="display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%; padding: 10px; background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2); color: #ffffff; font-size: 11px; font-weight: 700; font-family: monospace; letter-spacing: 1px; text-transform: uppercase; text-decoration: none; border-radius: 2px;">
                ...
                <div style="background: rgba(20, 20, 28, 0.7); border: 1px solid rgba(59, 130, 246, 0.4); padding: 16px; border-radius: 4px;">
                    ...
                    <?php if($inquiry->status === 'payment_verified' || $inquiry->status === 'scheduled_delivery' || $inquiry->status === 'delivered_completed'): ?>
                        <div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); padding: 12px; border-radius: 4px;">
                    ...
                    <?php elseif($inquiry->buyer_signed): ?>
                        <p style="font-size: 12px; color: #9ca3af; line-height: 1.5; margin-bottom: 14px;">
            ...
            <?php if($inquiry->notes): ?>
                <div>
                    ...
                    <p style="font-size: 13px; color: #9ca3af; line-height: 1.55;"><?php echo e($inquiry->notes); ?></p>
                </div>
            <?php endif; ?>
            ...
            <div>
                ...
                <div class="phase-timeline">
                    <?php $__currentLoopData = $phaseNames; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pi => $pname): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                            $pStatuses = $phaseStatuses[$pi];
                            $isActive = in_array($inquiry->status, $pStatuses);
                            $isDone = $currentIdx > $phaseThresholds[$pi];
                        ?>
                        <div class="phase-item <?php echo e($isActive ? 'active' : ($isDone ? 'done' : '')); ?>">
                            <div class="phase-dot <?php echo e($isActive ? 'active' : ($isDone ? 'done' : '')); ?>"></div>
                            <span class="phase-label"><?php echo e($pname); ?></span>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
        ...
        <div class="chat-area">
            ...
            <div class="messages-container" id="messagesContainer">
                ...
                <div id="deliveryManifestCard" style="display: none; width: 100%; max-width: 440px; margin: 0 auto 16px; background: rgba(249, 115, 22, 0.08); border: 1px solid rgba(249, 115, 22, 0.35); border-radius: 6px; padding: 16px; font-family: 'Inter', sans-serif;">
                    ...
                    <div style="display: flex; gap: 12px; align-items: center;">
                        <div style="width: 70px; height: 55px; border-radius: 4px; overflow: hidden; border: 1px solid rgba(255,255,255,0.15); flex-shrink: 0; background: #000;">
                            <img src="<?php echo e($carImg); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                        </div>
                        <div style="flex: 1; min-width: 0;">
                            <div style="font-family: 'Space Mono', monospace; font-size: 12px; font-weight: 700; color: #ffffff; text-transform: uppercase; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                <?php echo e($inquiry->car_model); ?>

                            </div>
                            <div style="font-size: 10px; color: #f97316; font-family: monospace; margin-top: 2px;">
                                ID: #APX-<?php echo e(str_pad($inquiry->id, 5, '0', STR_PAD_LEFT)); ?> &nbsp;·&nbsp; SKU: SUPERCAR-<?php echo e(strtoupper(substr(md5($inquiry->car_model ?? 'VIP'), 0, 6))); ?>

                            </div>
                            <?php if($inquiry->selected_config): ?>
                                <div style="font-size: 10px; color: #9ca3af; margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                    Spec: <?php echo e($inquiry->selected_config); ?>
                    ...
                    <div style="margin-top: 12px; padding-top: 8px; border-top: 1px dashed rgba(249,115,22,0.25); font-size: 11px; color: #cbd5e1; display: flex; flex-direction: column; gap: 4px;">
                        <div><strong style="color: #9ca3af;">Penerima:</strong> <?php echo e(auth()->user()->name); ?> (<?php echo e(auth()->user()->phone ?? $inquiry->phone); ?>)</div>
                        <div><strong style="color: #9ca3af;">Alamat Tujuan:</strong> <?php echo e(auth()->user()->address ?? 'Alamat Terverifikasi KYC Pembeli VIP'); ?></div>
                ...
                <?php else: ?>
                    <?php $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $isLoc = $msg->message && str_starts_with($msg->message, '__LOCATION__:'); ?>
                        <div class="message-group <?php echo e($msg->sender_type === 'buyer_delivery' ? 'buyer' : $msg->sender_type); ?>" data-id="<?php echo e($msg->id); ?>" data-channel="<?php echo e(in_array($msg->sender_type, ['driver', 'buyer_delivery']) ? 'delivery' : 'sales'); ?>">
                            <span class="message-sender" style="display:flex; align-items:center; gap:6px;">
                                <?php if($msg->sender_type === 'rm'): ?>
                                    <img src="<?php echo e(asset('images/logo/profile_sales.webp')); ?>" style="width:20px; height:20px; border-radius:50%; object-fit:cover;">
                                    <span style="color:#f87171; font-weight:700;">SALES RM</span> &mdash; <?php echo e($msg->sender_name); ?>

                                <?php elseif($msg->sender_type === 'driver'): ?>
                                    <img src="<?php echo e(asset('images/logo/profile_delivery.webp')); ?>" style="width:20px; height:20px; border-radius:50%; object-fit:cover;">
                                    <span style="color:#22d3ee; font-weight:700;">DRIVER ESCORT</span> &mdash; <?php echo e($msg->sender_name); ?>

                                <?php else: ?>
                                    <img src="<?php echo e(asset('images/logo/profile_user.webp')); ?>" style="width:20px; height:20px; border-radius:50%; object-fit:cover;">
                                    <span>PEMBELI</span> &mdash; <?php echo e($msg->sender_name); ?>
                            ...
                            <div class="message-bubble">
                                <?php if($isLoc): ?>
                                    ...
                                    ?>
                                    <div class="location-card" data-location="<?php echo e(trim($lat)); ?>,<?php echo e(trim($lng)); ?>" data-mapid="<?php echo e($mapId); ?>">
                                        <div class="loc-label"><i class="fa-solid fa-location-dot"></i> LOKASI DIKIRIM</div>
                                        <div id="<?php echo e($mapId); ?>" class="loc-map-render"></div>
                                        <a href="https://www.google.com/maps?q=<?php echo e(trim($lat)); ?>,<?php echo e(trim($lng)); ?>" target="_blank" class="loc-open-btn">
                                            <i class="fa-solid fa-arrow-up-right-from-square"></i> Buka di Google Maps
                                        </a>
                                    </div>
                                <?php else: ?>
                                    <?php if($msg->message): ?><span class="msg-text"><?php echo e($msg->message); ?></span><?php endif; ?>
                                    <?php if($msg->attachment): ?>
                                        <?php $attachPath = str_replace('\\', '/', $msg->attachment); $ext = strtolower(pathinfo($attachPath, PATHINFO_EXTENSION)); ?>
                                        <?php if(in_array($ext, ['jpg','jpeg','png','webp','gif'])): ?>
                                            <a href="<?php echo e(asset('storage/'.$attachPath)); ?>" target="_blank">
                                                <img src="<?php echo e(asset('storage/'.$attachPath)); ?>" style="max-width:240px; max-height:180px; border-radius:4px; border:1px solid rgba(255,255,255,0.2); display:block;">
                                            </a>
                                        <?php else: ?>
                                            <a href="<?php echo e(asset('storage/'.$attachPath)); ?>" target="_blank" style="display:inline-flex; align-items:center; gap:6px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); padding:6px 12px; color:#60a5fa; text-decoration:underline; font-size:11px; font-family:monospace;">
                            ...
                            <span class="message-time"><?php echo e($msg->created_at->format('d M, H:i')); ?></span>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
            ...
            <div id="salesChatInput" class="chat-input-area" style="display: <?php echo e(auth()->user()->isDelivery() ? 'none' : 'flex'); ?>;">
                ...
                <div class="chat-input-row">
                    ...
                    <textarea
                        id="messageInput"
                        class="chat-input"
                        placeholder="<?php echo e(auth()->user()->isRm() || auth()->user()->isManager() ? 'Tulis balasan ke buyer...' : 'Tulis pesan ke Sales RM atau lampirkan bukti transfer...'); ?>"
                        rows="2"
                    ></textarea>
            ...
            <?php if(auth()->user()->isDelivery()): ?>
            <div id="driverChatInput" class="chat-input-area" style="display: <?php echo e(auth()->user()->isDelivery() ? 'flex' : 'none'); ?>; border-top-color: rgba(249,115,22,0.3); background: rgba(8,8,16,0.95);">
    ...
    <script>
        const USER_ROLE = '<?php echo e(auth()->user()->role); ?>';
        const INQUIRY_ID = <?php echo e($inquiry->id); ?>;
        const POLL_URL = '<?php echo e(route('portal.message.poll', $inquiry)); ?>';
        const SEND_URL = '<?php echo e(route('portal.message.store', $inquiry)); ?>';
        ...
        const TRACKING_URL = '<?php echo e(route('portal.tracking', $inquiry)); ?>';
...
<?php /**PATH C:\xampp\htdocs\apex-automotive\resources\views/portal/consultation.blade.php ENDPATH**/ ?>